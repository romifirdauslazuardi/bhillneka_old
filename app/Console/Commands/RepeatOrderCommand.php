...
class RepeatOrderCommand extends Command
{
        ...
        try {
            ...
            foreach($orders as $index => $order){
                ...
                if(empty($order->repeat_order_at) && $checkOtherOrder == 0){
                    $date30plusDay = date("Y-m-d",strtotime($order->created_at." + " . (($order->repeat_interval !== NULL ? $order->repeat_interval : 1) * 30) . " day"));
                ...
                if(!empty($order->repeat_order_at) && $checkOtherOrder == 0){
                    $dateRepeatAt = date("Y-m",strtotime($order->created_at." + " . ($order->repeat_interval !== NULL ? $order->repeat_interval : 1) . " month"));
    ...
    private function generateOrder($order){
        ...
        $create = $create->create([
            ...
            'repeat_interval' => ($order->repeat_interval !== NULL ? $order->repeat_interval : 1),
